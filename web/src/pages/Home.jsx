...
function Home() {
    ...
    const featureCards = IS_READ_ONLY_MODE
        ? [
            { to: '/library', title: 'Organized Library', copy: 'Browse uploaded novels, chapter counts, and listening availability in one place.', icon: Library },
            { to: '/library', title: 'Premium Reading', copy: 'Tune reader settings once and carry them across chapters and playback.', icon: Headphones },
        ]
        : [
            { to: '/scraper', title: 'Fast Scraping', copy: 'Automatically fetch new novels and keep your library updated locally.', icon: Sparkles },
            { to: '/library', title: 'Organized Library', copy: 'Keep your novels, chapter counts, and playback settings easy to revisit.', icon: Library },
            { to: '/library', title: 'Premium Reading', copy: 'Tune reader settings once and reuse them across chapters, audio, and theme changes.', icon: Headphones },
        ];
                ...
                <motion.div className="flex-1 flex flex-col gap-6 md:gap-8 min-w-0" variants={staggerContainer} initial="hidden" animate="show">
                    <motion.div variants={fadeInUp} className="pt-10 md:pt-20 pb-8 md:pb-16 px-4 md:px-8 relative">
                        ...
                        <div className="relative z-10 max-w-2xl">
                            <span className="inline-block mb-4 px-4 py-1.5 rounded-full border border-stone-300/40 dark:border-violet-500/20 bg-white/30 dark:bg-violet-900/40 text-stone-700 dark:text-violet-200 text-xs font-bold tracking-[0.2em] uppercase backdrop-blur-md">{IS_READ_ONLY_MODE ? 'Reader-Only Library' : 'Local-First Reading Studio'}</span>
                            ...
                            <p className="text-stone-800 dark:text-violet-200/80 text-lg md:text-xl leading-relaxed mb-8 max-w-xl font-medium dark:font-normal drop-shadow-sm dark:drop-shadow-none">{IS_READ_ONLY_MODE ? 'Read and listen to the chapters and audio that have already been uploaded for this library.' : 'Scrape, collect, read, and listen to your favorite web novels locally with high-quality TTS.'}</p>
