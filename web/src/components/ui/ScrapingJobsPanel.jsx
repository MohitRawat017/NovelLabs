...
import { X, Loader, CheckCircle, XCircle, Trash2, Download, StopCircle, Headphones, Sparkles } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import './ScrapingJobsPanel.css';

function ScrapingJobsPanel() {
    const { jobs, audioJobs, isOpen, closePanel, removeJob, cancelJob } = useScrapingJobs();
    ...
    const isScraperJobActive = (status) => status === 'pending' || status === 'running' || status === 'detecting';
    const isAudioJobActive = (status) => status === 'queued' || status === 'pending' || status === 'generating';
                                ...
                                <div className="flex flex-col gap-6">
                                    ...
                                    <section className="flex flex-col gap-3">
                                        ...
                                        {audioJobs.length === 0 ? renderEmptySection('No audio jobs yet.') : (
                                            <div className="flex flex-col gap-3">
                                                {audioJobs.map((job) => (
                                                    <div key={job.job_id} className={`flex flex-col gap-3 p-4 rounded-[1.15rem] border ${job.provider === 'qwen3' ? 'border-amber-500/30 dark:border-amber-400/20' : 'border-blue-500/30 dark:border-blue-400/20'} bg-white/40 dark:bg-white/5`}>
                                                        <div className="flex items-start justify-between gap-3">
                                                            <div className="flex items-start gap-3 min-w-0">
                                                                <span className="flex items-center justify-center w-5 mt-0.5">{getStatusIcon(job.status)}</span>
                                                            ...
                                                            {job.provider && (
                                                                <span className={`inline-flex items-center px-2 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase ${job.provider === 'qwen3' ? 'bg-amber-100 text-amber-600 dark:bg-amber-500/10 dark:text-amber-400' : 'bg-blue-100 text-blue-600 dark:bg-blue-500/10 dark:text-blue-400'}`}>
                                                                    {job.provider === 'qwen3' ? 'Qwen' : 'Kokoro'}
                                                                </span>
                                                            )}
                                                        </div>
                                                        ...
                                                        {job.status !== 'failed' && (
                                                            <>
                                                                <div className="h-1.5 overflow-hidden rounded-full bg-stone-200/80 dark:bg-white/10">
                                                                    <motion.div
                                                                        className={`h-full rounded-full ${job.status === 'completed' ? 'bg-gradient-to-r from-emerald-400 to-emerald-500' : job.provider === 'qwen3' ? 'bg-gradient-to-r from-amber-400 to-amber-500' : 'bg-gradient-to-r from-blue-400 to-blue-500'}`}
                                                                        initial={{ width: 0 }}
                                                                ...
                                                                <div className="flex items-end justify-between gap-3 text-xs font-bold tracking-wider uppercase text-stone-500 dark:text-stone-400">
                                                                    <span className="flex flex-col gap-1">
                                                                        {job.total_chunks
                                                                            ...
                                                                            : isAudioJobActive(job.status)
                                                                                ? 'Preparing chunks...'
                                                                                : job.status}
                                                                        {job.estimated_remaining_seconds != null && isAudioJobActive(job.status) && (
